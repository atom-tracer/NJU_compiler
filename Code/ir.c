...
char *translate_Dec(TreeNode *root, Type type)
{
    ...
    if (compareName(root, 3, "VarDec", "ASSIGNOP", "Exp"))
    {
        ...
        sprintf(res, "%s%s%s := %s\n", code, code1, VarDec_id(root->child[0]), getVar(t1));
    }
    return res;
}
...
char *translate_Exp(TreeNode *root, Variable *place)
{
    ...
    if (compareName(root, 1, "ID"))
    {
        ...
        {
            if (!find_symbol_var(root->child[0]->id))
    ...
    if (compareName(root, 4, "Exp", "LB", "Exp", "RB"))
    {
        ...
        sprintf(res, "%s%s%s := %s * #%d\n%s := %s + %s\n%s := %s\n", code1, code2, getVar(t3), getVar(t2), size, getVar(t1), getVar(t1), getVar(t3), getVar(place), getVar(t1));
        place->is_pointer = true;
    ...
    if (compareName(root, 3, "Exp", "RELOP", "Exp") || compareName(root, 3, "Exp", "AND", "Exp") || compareName(root, 3, "Exp", "OR", "Exp") || compareName(root, 2, "NOT", "Exp"))
    {
        if (place == NULL)
        {
            char *code1 = translate_Exp(root->child[0], NULL);
            char *code2 = translate_Exp(root->child[2], NULL);
            res = malloc(strlen(code1) + strlen(code2) + 300);
            sprintf(res, "%s%s", code1, code2);
        }
        else
        {
            ...
            sprintf(res, "%s%s\nLABEL %s :\n%s := #1\nLABEL %s :\n", code0, code1, label1, getVar(place), label2);
        }
    }
    assert(res != NULL);
    return res;
