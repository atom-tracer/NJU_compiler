...
Type Exp(TreeNode *root)
{
    ...
    for(int i=0;i<7;i++){
        ...
        if(compareName(root,3,"Exp",op,"Exp")){
            ...
            if(type1==NULL||type2==NULL)
                return NULL;
            if(!compareType(type1,type2)||type1->kind!=BASIC||type2->kind!=BASIC){//类型不同或不是基本类型
    ...
    if(compareName(root, 4, "Exp", "LB", "Exp", "RB")){
        ...
        if(type1==NULL||type2==NULL)
            return NULL;
        if(type1->kind!=ARRAY){//不是数组
            add_semantic_error(10,root->line);
            return NULL;
        }
        if(type2->kind!=BASIC||type2->content.basic!=INT_TYPE){//下标不是整数
    ...
    if(compareName(root, 3, "ID", "LP", "RP")){
        ...
        if(type->kind!=FUNCTION){//不是函数
            add_semantic_error(11,root->line);
            return NULL;
        }
        if(type->content.func.tail!=NULL){//参数不匹配
    ...
    if(compareName(root, 3, "Exp", "ASSIGNOP", "Exp")){
        ...
        if(type1==NULL||type2==NULL)
            return NULL;
        if(!compareType(type1,type2)){//类型不同
