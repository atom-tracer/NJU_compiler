...
typedef struct IR_val {//表示IR中的值，可以是变量或常量
    bool is_const;//否为常量
    union {
        IR_var var;//如果是变量，则存储变量的标识符
        int const_val;//如果是常量，则直接存储常量的值
    };
} IR_val;
...
typedef struct {//基本块
    IR_label label;//基本块标签
    bool dead;
    List_IR_stmt_ptr stmts;//块中的语句列表
...
typedef struct IR_function{
    char *func_name;//函数名
    Vec_IR_var params;//参数列表
    Map_IR_var_IR_Dec map_dec; // dec_var => (addr_var, size)//变量声明表
    List_IR_block_ptr blocks;//基本块列表
    // Control Flow Graph
    IR_block *entry, *exit;//入口和出口
    Map_IR_label_IR_block_ptr map_blk_label; // Label -> Block 指针//标签到基本块的映射
...
typedef struct {
    CLASS_IR_stmt
    IR_RELOP_TYPE relop;//关系运算符
    union {
        IR_val use_vec[2];
        struct { IR_val rs1, rs2; };
    };
    IR_label true_label, false_label;//真假分支的标签
    IR_block *true_blk, *false_blk;//真假分支的基本块
} IR_if_stmt;
